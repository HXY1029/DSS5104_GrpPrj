...
def plot_predictions(actual, predicted, title, save_path=None):
    ...
    try:
        ...
        ax.plot(actual.index, actual.values, label='Actual Sales', linewidth=2, color='#2878B5')
        ax.plot(predicted.index, predicted.values, label='Predicted Sales', linewidth=2, color='#C82423', linestyle='--')
        
        # Set title and labels
        ax.set_title('Retail Sales Prediction Analysis', fontsize=16, pad=20)
...
def optimize_sarimax_params(train_data, exog_data):
    ...
    seasonal_pdq = [(x[0], x[1], x[2], 7) for x in list(itertools.product(p, d, q))]  # Use 7 days as seasonal period
...
def save_report(predictions, metrics, model_summary, feature_importance, output_dir):
    ...
    try:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("=== Retail Sales Prediction Analysis Report ===\n")
...
def retail_forecast(data_path):
    ...
    try:
        ...
        # First group by time and aggregate to avoid duplicate timestamps
        print("\nProcessing data...")
        ...
        coefficients = model.params[1:5]  # Skip intercept
        ...
        if timestamp:
            ...
            plot_predictions(test_data, predicted_mean, 'Retail Sales Prediction Results (with features)', save_path=plot_path)
...
if __name__ == "__main__":
    try:
        ...
        if retail_metrics:
            print("\nRetail Forecasting Evaluation Results:")
            for metric, value in retail_metrics.items():
                print(f"{metric}: {value:.4f}")
        else:
            print("Retail forecasting failed, please check data format")
