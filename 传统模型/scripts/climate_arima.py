...
def plot_predictions(actual, predicted, title, save_path=None):
    ...
    try:
        ...
        # 绘制实际值和预测值
        ax.plot(actual.index, actual.values, label='实际PM2.5浓度', linewidth=2, color='#2878B5')
        ax.plot(predicted.index, predicted.values, label='预测PM2.5浓度', linewidth=2, color='#C82423', linestyle='--')
        
        # 设置标题和标签（使用指定字体）
        if font is not None:
            ax.set_title('PM2.5浓度预测分析', fontproperties=font, fontsize=16, pad=20)
            ax.set_xlabel('时间', fontproperties=font, fontsize=12, labelpad=10)
            ax.set_ylabel('PM2.5浓度', fontproperties=font, fontsize=12, labelpad=10)
            legend = ax.legend(loc='best', prop=font, fontsize=12, frameon=True, framealpha=0.8)
        else:
            ax.set_title('PM2.5 Concentration Prediction', fontsize=16, pad=20)
...
def climate_forecast(data_path):
    ...
    try:
        ...
        print("\n数据集划分:")
        print(f"训练集: {len(train_data)} 条 ({train_data.index.min()} 到 {train_data.index.max()})")
        print(f"测试集: {len(test_data)} 条 ({test_data.index.min()} 到 {test_data.index.max()})")
        ...
        if timestamp:
            ...
            plot_predictions(test_data, predicted_mean, 'PM2.5浓度预测结果（带特征）', save_path=plot_path)
