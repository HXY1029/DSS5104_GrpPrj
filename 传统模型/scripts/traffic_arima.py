...
def plot_predictions(actual, predicted, title, save_path=None):
    ...
    try:
        ...
        ax.plot(actual.index, actual.values, label='Actual Orders', linewidth=2, color='#2878B5')
        ax.plot(predicted.index, predicted.values, label='Predicted Orders', linewidth=2, color='#C82423', linestyle='--')
        
        # Set title and labels
        ax.set_title('Traffic Order Prediction Analysis', fontsize=16, pad=20)
...
def save_report(predictions, metrics, model_summary, feature_importance, output_dir):
    ...
    try:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("=== Traffic Order Prediction Analysis Report ===\n")
...
def traffic_forecast(data_path):
    ...
    try:
        ...
        # Ensure pickup_hour column is datetime type
        ...
        if timestamp:
            ...
            plot_predictions(test_data, predicted_mean, 'Traffic Order Prediction Results (with features)', save_path=plot_path)
...
if __name__ == "__main__":
    try:
        ...
        if traffic_metrics:
            print("\nTraffic Forecasting Evaluation Results:")
            for metric, value in traffic_metrics.items():
                print(f"{metric}: {value:.4f}")
        else:
            print("Traffic forecasting failed, please check data format")
